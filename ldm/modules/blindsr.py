...
import ldm.modules.common as common
...
class DA_conv(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.silu(F.conv2d(x[0].view(1, -1, h, w), kernel, groups=b*c, padding=(self.kernel_size-1)//2))
...
class DAB(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.silu(self.da_conv1(x))
        out = self.silu(self.conv1(out))
        out = self.silu(self.da_conv2([out, x[1]]))
        out = self.conv2(out)    
        #out = self.da_conv1(x)
        return out
...
class degstepBlock(nn.Module):
    """
    Any module where forward() takes timestep embeddings as a second argument.
    """

    @abstractmethod
    def forward(self, x, deg_pre):
        """
        Apply the module to `x` given `emb` timestep embeddings.
        """
        
class DAG_chain(degstepBlock):
    def __init__(self, conv, n_feat, kernel_size, reduction, n_blocks):
        super(DAG_chain, self).__init__()
        self.n_blocks = n_blocks
        modules_body = [
            DAB(conv, n_feat, kernel_size, reduction) \
            for _ in range(n_blocks)
        ...
        (nn.Linear(1024, n_feat, bias=False)),
            nn.SiLU(),
        )
    def forward(self, x, deg_pre):
        '''
        ...
        '''
        res = x
        deg_pre = self.chain(deg_pre)
        for i in range(self.n_blocks):
            res = self.body[i]([res, deg_pre])
        res = self.body[-1](res)
        res =  x + res  
...
if __name__ == "__main__":
    # 初始化模型
    model = ResNet(layers= [3, 4, 6, 3], num_classes= 1024)
