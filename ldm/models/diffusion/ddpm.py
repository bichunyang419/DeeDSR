...
class DDPM(pl.LightningModule):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, cond=None):
        model_out = self.model.apply_model(x, t, cond)
        if self.parameterization == "eps":
    ...
    def noise_sample(self, x_start, t, x_t=None):
        #noise = default(noise, lambda: torch.randn_like(x_start))
        return ((x_t - extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start) /
...
class LatentDiffusion(DDPM):
    ...
    def p_sample_loop(self, cond, shape, return_intermediates=False,
        ...
        if x_T is None:
            ...
            img = self.q_sample(x_start=x_T, t=ts, noise=torch.randn_like(x_T))
            #img = x_T
